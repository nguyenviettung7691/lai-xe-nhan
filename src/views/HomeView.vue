...
const total = computed(() => topics.reduce((sum, topic) => sum + topic.lessons.length, 0))
const firstLesson = topics[0].lessons[0]
</script>

<template>
  <section class="hero">
    <div class="hero-copy"><p class="eyebrow">BÀI TẬP HÔM NAY</p><h1>Lái vững hơn,<br /><em>nhàn đầu hơn.</em></h1><p class="hero-text">Những kỹ năng thực tế được chia nhỏ để bạn học nhanh, nhớ lâu và xử lý bình tĩnh trên mọi cung đường.</p><RouterLink :to="`/lessons/${firstLesson.id}`" class="button primary">Bắt đầu học <span>→</span></RouterLink></div>
  ...
  <div class="topic-grid compact"><RouterLink v-for="topic in topics" :key="topic.id" :to="`/topics/${topic.id}`" class="topic-card" :style="{ '--accent': topic.color }"><span class="topic-icon">{{ topic.icon }}</span><strong>{{ topic.title }}</strong><small>{{ topic.lessons.length }} bài học</small></RouterLink></div>
